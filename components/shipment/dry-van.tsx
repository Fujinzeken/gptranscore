...
export function DVQuote() {
  ...
  return (
    <section id="quote" className="bg-ink px-gut py-[clamp(96px,15vh,190px)]">
      <Reveal>
        <div className="grid grid-cols-12 gap-x-[clamp(32px,5vw,88px)] gap-y-[clamp(36px,6vh,64px)] max-[1000px]:grid-cols-1">
          <div className="col-span-7 max-[1000px]:col-span-1">
            <p
            ...
            <h2
              ...
              className={cx(
                revealItem,
                "type-display m-0 mt-[clamp(18px,2.6vh,30px)] max-w-[15em]",
                "text-[clamp(26px,3.6vw,52px)] text-paper",
              )}
            >
              One load or weekly volume, you get a clear answer on
            ...
            <p
              ...
              className={cx(
                revealItem,
                "mt-5 max-w-[52ch] text-[clamp(15px,1.15vw,17.5px)] leading-[1.62] text-mute",
              )}
            >
              Send the lane and the details — we&rsquo;ll come back with a
              clear answer on availability.
            </p>

            <div
              style={{ "--i": 3 } as React.CSSProperties}
              className={cx(revealItem, "mt-9 flex flex-wrap gap-3")}
            >
              <button
                type="button"
                onClick={openQuote}
                className={cx(
                  ...
                  btnHero,
                  btnSolid,
                  "cursor-pointer shadow-[0_12px_34px_-12px_rgba(11,143,203,0.75)]",
                )}
              >
                Request a Quote
                <ArrowRight size={18} />
              </button>
              <a href="/services" className={cx(btn, btnHero, btnGhost)}>
          ...
          {/* What to send — the checklist straight out of the CSV copy, as a
              ruled manifest so the close rhymes with the rest of the page. */}
          <div className="col-span-5 max-[1000px]:col-span-1">
            <p
              style={{ "--i": 3 } as React.CSSProperties}
              className={cx(label, revealItem, "m-0 text-mute-2")}
            ...
            <ul
              className={cx(
                revealItem,
                "m-0 mt-[clamp(18px,2.6vh,30px)] list-none border-t border-rule",
              )}
              style={{ "--i": 4 } as React.CSSProperties}
            >
              {[
                ...
                "Pallet count and loading method",
                "Appointment times",
              ].map((item, i) => (
                <li
                  key={item}
                  style={{ "--i": i + 5 } as React.CSSProperties}
                  className="flex items-baseline gap-4 border-b border-rule py-4 text-[clamp(15px,1.2vw,18px)] leading-[1.5] text-paper"
                >
                  <span
                    className={cx(
                      label,
                      "m-0 shrink-0 text-mute-2 tabular-nums",
                    )}
                  >
                    {String(i + 1).padStart(2, "0")}
                  </span>
